...
export default class Seemore extends Component {
        ...
        <Container>
            ...
            <View style={styles.Thumbnail}>
              <ImageBackground style={styles.Thumbnail} source={require(a)}>
                <View>
                  ...
                  <Text
                    style={{fontSize: 14, paddingBottom: 20, paddingLeft: 70}}>
                    {this.state.title}
                  </Text>
                </View>
              </ImageBackground>
                      ...
                      <View
                        ...
                        <View style={{flex: 0.6}}>
                          <Text style={styles.TextCardItem}>
                            {this.state.totalDay / (1000 * 60 * 60 * 24)}
                          </Text>
                          <Text style={styles.TextCardItem}>
                            {this._subStr(this.state.gia)} ngày
                          </Text>
          ...
          <View style={styles.footer}>
            <ScrollView style={{flex: 1}} horizontal={true}>
              <Text style={{fontSize: 20, fontWeight: '100', paddingLeft: 20}}>
                Tổng cộng
              </Text>
              <Text
                style={{
            ...
            <View
              ...
              <Button block success style={styles.Button} onPress={this.Oders}>
                <Text
                  style={{
                    fontSize: 22,
                    fontWeight: 'bold',
  ...
  Oders = async () => {
    ...
    const {chosenDate, chosenDateTo, disabled, totalDay} = this.state;
    if (disabled === true) {
      Alert.alert('Cảnh báo','Bạn chưa chưa chọn ngày');
    } else if (totalDay <= 0) {
     Alert.alert('Cảnh báo','Bạn chưa chưa chọn ngày trả');
    } else {
      this.setState({
        lazyLoad: true,
      });
      fetch(HostName + 'api/Oders', {
        ...
        body: JSON.stringify({
          ...
          tongTien:
            this.state.gia * (this.state.totalDay / (1000 * 60 * 60 * 24)) +
            2500000,
          giamgia: null,
          huy: false,
          xacnhan: null,
        ...
        .then(responseJson => {
          if (responseJson.title == 'Not Found') {
            ...
            this.props.navigation.navigate('Oderproduct');
          }
        })
        .catch(error => {
          alert(error);
